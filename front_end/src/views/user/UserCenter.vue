<template>
    <main>
        <header>
            <div class="info">
                ...
                <div class="name">
                    ...
                    <span>
                        <el-icon v-if="userInfoStore.userInfo.sex == 1">
                            <Male />
                        </el-icon>
                        <el-icon v-else>
                            <Female />
                ...
                <span class="edit" @click="toEditUserInfo"><el-icon>
                        <Edit />
                    </el-icon>编辑个人信息</span>
                <span class="logout" @click="logoutHandler">退出登录</span>    
            </div>
            <nav>
                <ul class="tabs">
                    <li><a @click="changeTab('/user/shoppingCart')"
                            :class="[route.name == 'shoppingCart' ? 'active_tab' : '']">购物车</a></li>
                    <li><a @click="changeTab('/user/order')" :class="[route.name == 'order' ? 'active_tab' : '']">购买记录</a>
                    </li>
                    <!-- <li><a href="">买过的店</a></li> -->
                    <li v-if="userInfoStore.userInfo.userType == 1"><a @click="changeTab('/user/stores')"
        ...
        <DialogModule :show="dialogConfig.show" :title="dialogConfig.title" @close="dialogConfig.show = false"
            :showCancel="false" :buttons="dialogConfig.buttons">
            <div class="dialog-body">
                <el-form ref="formDataRef" :model="formData" :rules="rules" label-position="top">
                    <el-form-item label="请上传你的头像" prop="avatar">
...
import { getDetailUserInfo, editUserInfo, getUserInfo, logout } from '@/api/user'
...
const toEditUserInfo = async () => {
    ...
    if (resData.address && resData.address !== '') {
